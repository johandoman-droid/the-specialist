...
def scan(url: str):
    with sync_playwright() as p:
        ...
        stealth_sync(page) # THE STEALTH WRAP
        
        try:
            # Go to site and wait for the actual table content to appear
            page.goto(url, wait_until="networkidle", timeout=60000)
            
            # WAIT specifically for a horse link to appear in the table
            page.wait_for_selector("a[href*='/horse/']", timeout=15000)
            ...
            for row in rows:
                ...
                for td in cells:
                    ...
                    if "." in txt and (txt.startswith("0") or txt.startswith("1.")): 
                        try: nt = float(txt)
                        except: pass
                
                runners.append({"horse": horse, "mr": mr, "nt": nt})

            if not runners: return {"status": "error", "message": "No runners found after wait"}
            
            results = specialist_v3(runners)
            return {"status": "success", "results": results}
        except Exception as e:
            if browser: browser.close()
            return {"status": "error", "message": "Site Security Blocked Connection. Try again."}
...
async def get_ui():
    ...
    <body class="pb-24">
        <header class="p-8 border-b border-zinc-900 text-center">
            ...
            <p class="text-[10px] tracking-[0.4em] uppercase opacity-40 mt-1 font-bold text-zinc-500 italic">AI Racing Intelligence V3</p>
        </header>

        <div class="p-5 max-w-md mx-auto">
            <input id="urlInput" type="text" placeholder="PASTE RACE URL..." class="w-full bg-zinc-900 border border-zinc-800 p-4 rounded-xl text-xs mb-3 outline-none focus:ring-2 ring-red-600 text-white">
            ...
            <div id="loading" class="hidden mt-10 text-center">
                ...
                <p class="mt-4 text-[10px] text-red-500 font-bold uppercase tracking-widest">Bypassing Security Wall...</p>
            </div>
        ...
        <script>
            async function runScan() {
                ...
                try {
                    const res = await fetch(`/api/v1/scan?url=${encodeURIComponent(url)}`);
                    ...
                    } else { alert(data.message || 'Error: Site Blocked the Engine'); }
